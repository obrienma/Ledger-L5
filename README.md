...
- [ ] **Phase 10 — Automatic invoice email delivery:** `POST /invoices/{id}/issue` gains a third side effect — emailing the rendered PDF bytes (not the R2 copy) to `customer.email` via Resend as an attachment, recording `invoices.sent_at`/`sent_to_email` on success. Unlike R2 upload, a failed send blocks the entire transition and rolls it back — `issued` is documented to mean "the customer has been notified," so send success joins `transition_status` as part of the financial-authority boundary rather than a degrading downstream call. The issue form lets an operator edit `customer.email` inline, which persists to `customers` — the first code path in the app that mutates a customer row. ([ADR 0016](docs/adr/0016-automatic-invoice-email-delivery.md) — Accepted)
